...
rtma_gust = rtma_data['Wind_speed_gust_Analysis_height_above_ground'].metpy.sel(time=dt, method='nearest').squeeze()
...
cbar_gust1.set_label(label="Wind Gust (mph)", size=12, fontweight='bold')
...
cbar_gust2.set_label(label="Wind Gust (mph)", size=12, fontweight='bold')
...
# Saves figure
fig.savefig(f"Weather Data/RTMA Red Flag Criteria Based on Wind Gust North")
fig1.savefig(f"Weather Data/RTMA Red Flag Criteria Based on Wind Gust Central")
fig2.savefig(f"Weather Data/RTMA Red Flag Criteria Based on Wind Gust South")
